...
import {View,Text,StyleSheet,TouchableOpacity,ActivityIndicator,ToastAndroid,Platform,PermissionsAndroid, Alert } from 'react-native'
...
const WelcomeScreen = () => {
    ...
    useEffect(()=>{
     ...
     try{
      ...
      : (Platform.Version >= 33 ? PERMISSIONS.ANDROID.READ_MEDIA_AUDIO : PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE);

        const value = await request(permission);
        console.log(Platform.Version,'value is: ',value)
      }
        catch(e){
          ToastAndroid.show(`Permission denied in useEffect  -  ${e}`, ToastAndroid.SHORT);
      ...
      const fetchData = async () => {
        try {
         ...
         if (!dataCheck) {
            ...
            console.log('lat',dataIsAvail[0].Latitude);
            console.log('longi',dataIsAvail[0].Longitude);
        ...
        try {
          ...
          if (granted === 'granted') {
            ...
            setLoading(false)
             ...
             ToastAndroid.show('Requesting permission got problem', ToastAndroid.SHORT);
          ...
          try {
             await RNFS.downloadFile({
              ...
              console.log('download success',result)
               ...
               setLoading(false)
               navigation.navigate('PlaceDetails')
            }).catch((error:any)=>{console.log('error to download',error)});
          } catch (error) {
          ...
          if (response) {
            Geolocation.getCurrentPosition(
              position => {
                ...
                Alert.alert(apiData[0].Latitude,apiData[0].Longitude)
               if(position.coords.latitude === apiData[0].Latitude && apiData[0].Longitude)
                {
                  navigation.navigate('PlaceDetails')
                  
                }
                else {
                  Alert.alert('You are not in Khazana Mahal');
    ...
    return <View style={{height:"100%",alignItems:'center',justifyContent:'center'}}>
        { showLoading && 
             ...
             {
                !showLoading  &&   
              <TouchableOpacity style={[style.button, { backgroundColor: colors.appBackgroundColor,marginTop:25,elevation:5 }]}
               onPress={() => {
                      setLoading(true) 
                      permission()  
